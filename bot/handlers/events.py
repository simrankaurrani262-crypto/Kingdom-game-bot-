...
def decision_event_keyboard(event_id: str):
    """Create keyboard for decision events.
    
    This function was missing from keyboards.py - added here to avoid import errors.
    """
    # Find the event to get its choices
    event = None
    ...
    buttons = []
    for choice_key, choice_data in event['choices'].items():
        label = choice_data if isinstance(choice_data, str) else choice_data.get('label', choice_key.title())
        buttons.append([InlineKeyboardButton(label, callback_data=f"decision:{event_id}:{choice_key}")])
...
async def process_decision(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if 'infantry' in outcome:
        ...
        if army:
            ...
            db.update_army(user_id, infantry=max(0, new_inf))
